...
(function (global, factory) {
  ...
  let global_stake = -1 // 插桩key根据这个值阻断react执行
  const STAKE_MAP = {
    '__rootFiberCreate': 0, // 在rootFiber创建后插桩
    '__beginWork': 10, // 在完成所有的beginWork进行插桩
  ...
  function finishEventHandler() {
    ...
    if (controlledComponentsHavePendingUpdates) {
      ...
      flushDiscreteUpdatesImpl("finishEventHandler");
  ...
  function discreteUpdates(fn, a, b, c, d) {
    if(['click'].includes(a)) {
       logger('discreteUpdates--事件'+ a, {
          domEventName: a, eventSystemFlags: b, container: c, nativeEvent: d
        })
    }
  ...
  function flushDiscreteUpdatesIfNeeded(timeStamp) {
    // logger("flushDiscreteUpdatesIfNeeded")
    {
      if (!isInsideEventHandler) {
        flushDiscreteUpdatesImpl("flushDiscreteUpdatesIfNeeded");
      }
    }
  }
  ...
  function dispatchDiscreteEvent(domEventName, eventSystemFlags, container, nativeEvent) {
    // logger("dispatchDiscreteEvent这个过程在冒泡所以会打印很多",{
    //   domEventName,
    //   eventSystemFlags,
    //   container: "id:" + container.id + "tag:" + container.tagName + container.className
    // })
    {
  ...
  function addTrappedEventListener(targetContainer, domEventName, eventSystemFlags, isCapturePhaseListener, isDeferredListenerForLegacyFBSupport) {
    // logger("addTrappedEventListener", {
    //   targetContainer, domEventName, 
    //   eventSystemFlags, 
  ...
  function createInstance(type, props, rootContainerInstance, hostContext, internalInstanceHandle) {
    ...
    logger("createInstance创建一个DOM--" + domElement.tagName, {
  ...
  // 凡是flush都会与异步调度有关，这里通过immediateQueueCallbackNode来控制是立刻执行还是异步调度
  function flushSyncCallbackQueue(__call, __isBanLog) {
    if(!['batchedEventUpdates$1',"flushPendingDiscreteUpdates",""].includes(__call) && !__isBanLog) {
       logger('flushSyncCallbackQueue--'+ __call, {
      __call
    })
    }
  ...
  */
...

  {
    ...
    HooksDispatcherOnMountInDEV = {
      ...
      useCallback: function (callback, deps) {
        ...
        const ref = mountCallback(callback, deps);
        logger('useCallback-mount-Hooks调用', {
          callback, deps,
          fiber:  currentlyRenderingFiber$1,// 纯粹打印添加的代码
          workInProgressHook: workInProgressHook,
          'fiber.memoizedState': currentlyRenderingFiber$1.memoizedState,
          'fiber.memoizedState.next': currentlyRenderingFiber$1.memoizedState ? currentlyRenderingFiber$1.memoizedState.next: null
      ...
      useEffect: function (create, deps) {
        ...
        mountHookTypesDev();
        checkDepsAreArrayDev(deps);
        const ref =  mountEffect(create, deps);
        logger('useEffect-mount-Hooks调用', {
          create, deps,
          fiber:  currentlyRenderingFiber$1,// 纯粹打印添加的代码
          workInProgressHook: workInProgressHook,
          'fiber.memoizedState': currentlyRenderingFiber$1.memoizedState,
      ...
      useLayoutEffect: function (create, deps) {
        ...
        mountHookTypesDev();
        checkDepsAreArrayDev(deps);
        const ref = mountLayoutEffect(create, deps);
         logger('useLayoutEffect-mount-Hooks调用', {
          create, deps,
          fiber:  currentlyRenderingFiber$1,// 纯粹打印添加的代码
          workInProgressHook: workInProgressHook,
          'fiber.memoizedState': currentlyRenderingFiber$1.memoizedState,
      ...
      useRef: function (initialValue) {
        ...
        mountHookTypesDev();
        const ref =  mountRef(initialValue); // 这里为了打印修改了一下代码
        logger('useRef-mount-Hooks调用', {
          initialValue,
          fiber:  currentlyRenderingFiber$1,// 纯粹打印添加的代码
          workInProgressHook: workInProgressHook,
          'fiber.memoizedState': currentlyRenderingFiber$1.memoizedState,
      ...
      useState: function (initialState) {
        ...
        try {
          const ref =  mountState(initialState);
          logger('useState-mount-Hooks调用', {
              initialState,
              fiber:  currentlyRenderingFiber$1,// 纯粹打印添加的代码
              workInProgressHook: workInProgressHook,
              'fiber.memoizedState': currentlyRenderingFiber$1.memoizedState,
    ...
    HooksDispatcherOnUpdateInDEV = {
      ...
      useEffect: function (create, deps) {
        logger('useEffect-update-Hooks调用', {
          create, deps
        })
        currentHookNameInDev = 'useEffect';
        updateHookTypesDev();
      ...
      useLayoutEffect: function (create, deps) {
        logger('useLayoutEffect-update-Hooks调用', {
          create, deps
        })
        currentHookNameInDev = 'useLayoutEffect';
        updateHookTypesDev();
      ...
      useState: function (initialState) {
        logger('useState-update-Hooks调用', {
          initialState
        })
        currentHookNameInDev = 'useState';
        updateHookTypesDev();
  ...
  function updateHostComponent(current, workInProgress, renderLanes) {
    const logFiber = current || workInProgress
     const name = logFiber ?   (typeof logFiber.type === 'function' ? logFiber.type.name :logFiber.type )     : '无null'
    logger('updateHostComponent---' + name, {
  ...
  function beginWork(current, workInProgress, renderLanes) {
    const name = workInProgress ?   (typeof workInProgress.elementType === 'function' 
    ? workInProgress.elementType.name :workInProgress.elementType )     : '无null'
    logger("beginWork ---" + name, {
      current, 
      workInProgress, 
      renderLanes
    })
  ...
  function completeWork(current, workInProgress, renderLanes) {
    ...
    const logFiber = current || workInProgress
    const name = logFiber ?   (typeof logFiber.type === 'function' ? logFiber.type.name :logFiber.type )     : '无null'
    logger('completeWork --' + name, {
      current, 
      workInProgress, 
      renderLanes,
      'workInProgress.tag': workInProgress.tag
    })
  ...
  function commitHookEffectListUnmount(tag, finishedWork) {
    logger('commitHookEffectListUnmount-更新function节点', {
      tag,
      finishedWork,
      '执行useEffect的destory': `
        do {
...
这就是useLayoutEffect与useEffect的区别。 ？？？？？
  ...
  */
  ...
  function commitLifeCycles(finishedRoot, current, finishedWork, committedLanes) {
    ...
    const $$type =(typeof finishedWork.type) === 'string' 
    ? finishedWork.type :finishedWork.type 
      ? finishedWork.type.name : 'type===null'
    logger('commitLifeCycles--' + $$type, {
      "finishedWork.tag": finishedWork.tag,
      ...
      case ClassComponent:
        {
          ...
          if (finishedWork.flags & Update) {
            // 通过current === null?区分是mount还是update，
            //调用componentDidMount (opens new window)或componentDidUpdate (opens new window)
  ...
  function commitAttachRef(finishedWork) {
    ...
    if (ref !== null) {
      ...
      const $$type = parseFiber(finishedWork)
      logger('commitAttachRef--'+$$type+'--处理ref',{
        '过程:': ' var instance = finishedWork.stateNode;',
        'ref.current': instanceToUse
      })
      // Moved outside to ensure DCE works with this flag
      // 这样就说明有两种使用方式 函数和直接将带有current的ref赋值
      if (typeof ref === 'function') {
        ref(instanceToUse);
      } else {
        {
  ...
  function commitPlacement(finishedWork) {
    const $$type = parseFiber(finishedWork)
    logger('commitPlacement--'+$$type+'--此DOM需要插入页面', {
      finishedWork
    })
  ...
  function insertOrAppendPlacementNodeIntoContainer(node, before, parent) {
    logger('插入到dom中', {
      '函数名:': "insertOrAppendPlacementNodeIntoContainer",
      node, before, parent
    })
    var tag = node.tag;
  ...
  function commitDeletion(finishedRoot, current, renderPriorityLevel) {
    logger('commitDeletion--该Fiber节点对应的DOM节点需要从页面中删除', {
  ...
  function scheduleUpdateOnFiber(fiber, lane, eventTime) {
    logger("scheduleUpdateOnFiber", {
      fiber, lane, eventTime,
      "markRootUpdated": "调用",
      "条件": (executionContext & LegacyUnbatchedContext) !== NoContext && // Check if we're not already rendering
      (executionContext & (RenderContext | CommitContext)) === NoContext,
      "executionContext === NoContext": executionContext === NoContext
    })
    ...
    if (lane === SyncLane) {
      ...
      (executionContext & (RenderContext | CommitContext)) === NoContext) {
        ...
        if (executionContext === NoContext) {
          ...
          resetRenderTimer();
          flushSyncCallbackQueue('scheduleUpdateOnFiber');
        }
      }
    } else {
  ...
  function performConcurrentWorkOnRoot(root) {
    ...
    var didFlushPassiveEffects = flushPassiveEffects("performConcurrentWorkOnRoot");
  ...
  function performSyncWorkOnRoot(root) {
    ...
    flushPassiveEffects("performSyncWorkOnRoot");
    var lanes;
    var exitStatus;
    ...
    root.finishedWork = finishedWork;
    root.finishedLanes = lanes;
    if(global_stake >= STAKE_MAP.__beginWork) { // 被插桩阻断执行
       
          console.error('被插桩阻断执行')
          return null;
    }
  ...
  function flushRoot(root, lanes) {
    ...
    if ((executionContext & (RenderContext | CommitContext)) === NoContext) {
      resetRenderTimer();
      flushSyncCallbackQueue('flushRoot');
    }
  }
  function getExecutionContext() {
    return executionContext;
  }
  function flushDiscreteUpdates(call) {
    // logger("flushDiscreteUpdates", {
    //   '调用父级': call
    // })
    ...
    flushPassiveEffects("flushDiscreteUpdates");
  }
  ...
  function flushPendingDiscreteUpdates() {
    ...
    flushSyncCallbackQueue('flushPendingDiscreteUpdates');
  }
  ...
  function batchedUpdates$1(fn, a) {
    ...
    try {
      ...
      if (executionContext === NoContext) {
        ...
        resetRenderTimer();
        flushSyncCallbackQueue('batchedUpdates$1');
      }
    }
  }
  function batchedEventUpdates$1(fn, a) {
    ...
    try {
      ...
      if (executionContext === NoContext) {
        // Flush the immediate callbacks that were scheduled during this batch
        resetRenderTimer();
        flushSyncCallbackQueue('batchedEventUpdates$1');
      }
    }
  }
...

    {
      try {
        ...
        if (executionContext === NoContext) {
          ...
          flushSyncCallbackQueue('discreteUpdates$1', !['click'].includes(a));
        }
      }
    }
  }
  function unbatchedUpdates(fn, a) {
    ...
    try {
      ...
      if (executionContext === NoContext) {
        // Flush the immediate callbacks that were scheduled during this batch
        resetRenderTimer();
        flushSyncCallbackQueue('unbatchedUpdates');
      }
    }
  }
  function flushSync(fn, a) {
...

    {
      try {
        ...
        flushSyncCallbackQueue('flushSync');
      }
    }
  }
...

    {
      try {
        ...
        if (executionContext === NoContext) {
          // Flush the immediate callbacks that were scheduled during this batch
          resetRenderTimer();
          flushSyncCallbackQueue('flushControlled');
        }
      }
    }
  }
  ...
  function prepareFreshStack(root, lanes) {
    ...
    logger("prepareFreshStack", {
      root,
      lanes,
      '做了一些准备动作': `  workInProgressRoot = root;
  ...
  function commitRoot(root) {
    ...
    var renderPriorityLevel = getCurrentPriorityLevel();
     logger("commitRoot", {
      ...
      " commitRootImpl.bind": "commitRootImpl.bind(null, root, renderPriorityLevel));"
    })
  ...
  function commitRootImpl(root, renderPriorityLevel) {
    logger('commitRootImpl', {
      renderPriorityLevel,
      "第一步": `
       触发useEffect回调与其他同步任务。由于这些任务可能触发新的渲染，所以这里要一直遍历执行直到没有任务
         do {
            flushPassiveEffects("commitRootImpl");
        } while (rootWithPendingPassiveEffects !== null);
      `,
      "before阶段": "commitBeforeMutationEffects",
  ...
  // 这样才能保证有effect的fiber都在effectList中
    ...
    if (hasUncaughtError) {
     ...
     logger('执行到这里commitRootImpl执行结束，理论上同步任务已经结束了')
    ...
    flushSyncCallbackQueue('commitRootImpl');
  ...
  */
  function commitBeforeMutationEffects() {
    logger('commitBeforeMutationEffects', {
      nextEffect
    })
    while (nextEffect !== null) {
  ...
  */
  function commitMutationEffects(root, renderPriorityLevel) {
    ...
    while (nextEffect !== null) {
      ...
      const $$type = parseFiber(nextEffect)
       logger('commitMutationEffects内部循环--' + $$type, {
        primaryFlags,
        Placement,
        PlacementAndUpdate,
        Update,
...
#
  ...
  */
  ...
  function flushPassiveEffects(call) {
    // logger("flushPassiveEffects", {
    //   '调用父级': call
    // })
  ...
  function enqueuePendingPassiveHookEffectMount(fiber, effect) {
    ...
    if (!rootDoesHavePassiveEffects) {
      rootDoesHavePassiveEffects = true;
      scheduleCallback(NormalPriority$1, function () {
        flushPassiveEffects('enqueuePendingPassiveHookEffectMount');
        return null;
      });
    }
  }
  function enqueuePendingPassiveHookEffectUnmount(fiber, effect) {
    ...
    if (!rootDoesHavePassiveEffects) {
      rootDoesHavePassiveEffects = true;
      scheduleCallback(NormalPriority$1, function () {
        flushPassiveEffects("enqueuePendingPassiveHookEffectUnmount");
        return null;
      });
    }
  }
  ...
  */
  function flushPassiveEffectsImpl() {
    ...
    // 如果存在同步任务，不需要等待下次事件循环的宏任务，提前执行他
    executionContext = prevExecutionContext;
    flushSyncCallbackQueue('flushPassiveEffectsImpl'); // If additional passive effects were scheduled, increment a counter. If this
  ...
  var scheduleRefresh = function (root, update) {
    {
      ...
      flushPassiveEffects("scheduleRefresh");
      flushSync(function () {
  ...
  var scheduleRoot = function (root, element) {
    {
      ...
      flushPassiveEffects("scheduleRoot");
      flushSync(function () {
  ...
  // __type增加一个__type为了打印类型
  var createFiber = function (tag, pendingProps, key, mode, __type) {
    // logger("createFiber", {
    //   tag,
    //   mode,
    //   pendingProps,
    //   type: __type,
    //   "FiberNode": "new FiberNode(tag, pendingProps, key, mode);"
    // })
  ...
  key, pendingProps, owner, mode, lanes) {
    ...
    var resolvedType = type;
  //   logger('createFiberFromTypeAndProps', {
  ...
  //   })
    ...
    var fiber = createFiber(fiberTag, pendingProps, key, mode, type || resolvedType);
  ...
  function createFiberFromElement(element, mode, lanes) {
    ...
    const $$type =(typeof fiber.type) === 'string' ? fiber.type : fiber.type.name
     logger("createFiberFromElement创建一个fiber--" + $$type, {
      element, mode, lanes,fiber
    })
  ...
  function createFiberRoot(containerInfo, tag, hydrate, hydrationCallbacks) {
    ...
    // logger('createFiberRoot', {
    //   "FiberRootNode": `var root = new FiberRootNode(containerInfo, tag, hydrate);`,
    //   "初始化": ` var uninitializedFiber = createHostRootFiber(tag);
  ...
  function createContainer(containerInfo, tag, hydrate, hydrationCallbacks) {
    ...
    const root =  createFiberRoot(containerInfo, tag, hydrate);
     logger('createContainer创建FiberRootNode', {
        root
    })
    return root
  }
  function updateContainer(element, container, parentComponent, callback) {
    logger('updateContainer调用', {
      "onScheduleRoot": "调用",
      "createUpdate": '调用',
      "scheduleUpdateOnFiber": "调用",
    })
    {
  ...
  function createRootImpl(container, tag, options) {
     logger("createRootImpl调用", {
      "listenToAllSupportedEvents": "处理事件相关",
      "createContainer":"创建root"
    })
  ...
  function createLegacyRoot(container, options) {
    logger("createLegacyRoot", {
      options,
      "创建Root": "new ReactDOMBlockingRoot"
    })
  ...
  function legacyCreateRootFromDOMContainer(container, forceHydrate) {
    ...
    logger('legacyCreateRootFromDOMContainer调用', {
      "forceHydrate": forceHydrate,
      '递归清理container中的节点': ` while (rootSibling = container.lastChild) {
        container.removeChild(rootSibling);
      }`,
      "createLegacyRoot": '调用'
    })
  ...
  function legacyRenderSubtreeIntoContainer(parentComponent, children, container, forceHydrate, callback) {
    ...
    var fiberRoot;
    logger('legacyRenderSubtreeIntoContainer调用', {
      "container._reactRootContainer": container._reactRootContainer|| "空",
  ...
  function render(element, container, callback, isBanLog) {
    if (!isValidContainer(container)) {
      {
        throw Error( "Target container is not a DOM element." );
      }
    }
    const _log = window.logger
    if(isBanLog) {
      window.logger = () => null;
    }
    ...
    console.log('执行render函数', element)
    {
      var isModernRoot = isContainerMarkedAsRoot(container) && container._reactRootContainer === undefined;
