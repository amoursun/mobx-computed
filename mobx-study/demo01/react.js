...
(function (global, factory) {
  ...
  typeof MessageChannel !== 'function') {
    ...
    requestHostCallback = function (cb) {
      logger('requestHostCallback--setTimeout版本')
      if (_callback !== null) {
    ...
    var performWorkUntilDeadline = function () {
      logger('performWorkUntilDeadline这是异步调度内容被执行', {
    ...
    requestHostCallback = function (callback) {
      ...
      logger('requestHostCallback--postMessage版本', {
  ...
  function workLoop(hasTimeRemaining, initialTime) {
    ...
    while (currentTask !== null && !(enableSchedulerDebugging )) {
      logger('workLoop循环--异步调度', {
        currentTask,
        'currentTask.callback即任务实体': currentTask.callback
      })
    ...
    if (currentTask !== null) {
      ...
      if (firstTimer !== null) {
        // 这里是一个setTimeout实现的，那么也就是即使这里的firstTimer.startTime - currentTime === 0
        // 代码也不会立刻执行，因为这是一个异步任务，需要等待同步代码执行完毕
        requestHostTimeout(handleTimeout, firstTimer.startTime - currentTime);
      }
  ...
  function unstable_scheduleCallback(priorityLevel, callback, options) {
    ...
    var newTask = {
      id: taskIdCounter++,
      callback: () => {console.log('callback被调用',callback.name); callback()},
    ...
    logger('unstable_scheduleCallback进行异步调度', {
      '新创建的task': newTask,
      callback: callback,
      'startTime > currentTime': startTime > currentTime ? 'true' : 'false',
      '!isHostCallbackScheduled && !isPerformingWork': !isHostCallbackScheduled && !isPerformingWork ? 'true': 'false'
    })
