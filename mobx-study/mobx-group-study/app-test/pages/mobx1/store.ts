...
class ObservableValue {
    ...
    set(value: string) {
        // 
         ...
         this.setNewValue_(value)
    }

    setNewValue_(newValue: string) {
...
function propagateChanged(observable: ObservableValue) {
     observable.observes.forEach((d: Reaction) => {
        d.onBecomeStale_()
    })
}


export function reportObserved(observable: ObservableValue): boolean {
     const derivation = _global.trackingDerivation
      if (derivation) {
            derivation!.observings.push(observable)
      }
      return false;
