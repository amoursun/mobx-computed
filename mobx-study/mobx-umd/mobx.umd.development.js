(function (global, factory) {
    ...
    function storeDecorator(target, property, type, arg_) {
      ...
      logger('storeDecorator添加mobx的annotation--' +property + '--' + type, {
        target, property, type, arg_
      })
      map[property] = {
        annotationType_: type,
        arg_: arg_
    ...
    var Atom = /*#__PURE__*/function () {
      ...
      _proto.reportChanged = function reportChanged() {
        startBatch();
        logger('reportChanged--设置属性', {
         ' globalState.inBatch':globalState.inBatch
        })
    ...
    var nextActionId = 1;
    var isFunctionNameConfigurable = (_getDescriptor$config = (_getDescriptor = /*#__PURE__*/getDescriptor(function () {}, "name")) === null || _getDescriptor === void 0 ? void 0 : _getDescriptor.configurable) !== null && _getDescriptor$config !== void 0 ? _getDescriptor$config : false;
    ...
    function createAction(actionName, fn, autoAction, ref) {
      logger('createAction--action对象创建', {
        actionName, fn, autoAction, ref
      })
    ...
    var ObservableValue = /*#__PURE__*/function (_Atom) {
      ...
      function ObservableValue(value, enhancer, name_, notifySpy, equals) {
        const name = name_.substring(name_.indexOf('@') + 1)
        logger('ObservableValue--'+name+'的ov对象创建', {
          value, enhancer, name_, notifySpy, equals
        })
        var _this;
        if (name_ === void 0) {
      ...
      _proto.prepareNewValue_ = function prepareNewValue_(newValue) {
        ...
        logger('OV.prepareNewValue_--设置属性',{
          newValue, 
           value_:this.value_, name_:this.name_
        } )
        return  comparer["default"](this.value_, newValue) ? globalState.UNCHANGED : newValue;
      };
      _proto.setNewValue_ = function setNewValue_(newValue) {
        var oldValue = this.value_;
        this.value_ = newValue;
        logger('OV.setNewValue_--设置属性--值：'+ newValue, {
          oldValue, newValue
        } )
        this.reportChanged();
    ...
    var ComputedValue = /*#__PURE__*/function () {
      ...
      function ComputedValue(options) {
        logger('ComputedValue--computed对象创建', {
          options
        })
...
*/
    ...
    function shouldCompute(derivation) {
      logger('CV.shouldCompute', {
        // derivation
        'derivation.dependenciesState_': derivation.dependenciesState_
      })
    ...
    */
    function trackDerivedFunction(derivation, f, context) {
      logger('trackDerivedFunction--处理监听关系', {
        // derivation, f, context,
        // 'globalState.trackingDerivation': globalState.trackingDerivation
      })
      var prevAllowStateReads = allowStateReadsStart(true);
    ...
    function bindDependencies(derivation) {
      logger('bindDependencies--反向将R对象绑定到OV对象的observ_中', {
        // derivation
      })
    ...
    function endBatch() {
      logger('endBatch--调度'+ '当前inBatch=' + globalState.inBatch)
      if (--globalState.inBatch === 0) {
        runReactions();
    ...
    function reportObserved(observable) {
      ...
      if (derivation !== null) {
         ...
         */
        if (derivation.runId_ !== observable.lastAccessedBy_) {
          logger('reportObserved--观察者-'+derivation.name_   +'被观察者-' + observable.name_, {
          // derivation,
          // observable,
          // 'derivation.newObserving_': derivation.newObserving_,
          // 'observable.lastAccessedBy_': observable.lastAccessedBy_,
          'derivation.runId_': derivation.runId_
         })
          observable.lastAccessedBy_ = derivation.runId_;
          // Tried storing newObserving, or observing, or both as Set, but performance didn't come close...
          derivation.newObserving_[derivation.unboundDepsCount_++] = observable;
    ...
    function propagateChanged(observable) {
      ...
      observable.observers_.forEach(function (d) {
        logger('propagateChanged--设置属性-循环触发监听者回调: ' + d.name_, {
          'observable.observers_': observable.observers_,
          d
        })
    ...
    var Reaction = /*#__PURE__*/function () {
      ...
      _proto.onBecomeStale_ = function onBecomeStale_() {
        logger('onBecomeStale_--衍生对象调度')
        this.schedule_();
      };
      _proto.schedule_ = function schedule_() {
        logger('schedule_--衍生对象调度', {
          isScheduled_: this.isScheduled_,
          "globalState.pendingReactions": globalState.pendingReactions,
        })
      ...
      _proto.runReaction_ = function runReaction_() {
        logger('runReaction_--衍生对象调度', {
          isDisposed_: this.isDisposed_
        })
        if (!this.isDisposed_) {
          startBatch();
          this.isScheduled_ = false;
    ...
    function runReactions() {
      const flag = globalState.inBatch > 0 || globalState.isRunningReactions
      logger('runReactions--'+ (flag ? '停止执行 ': '继续向下 ') + 'inBatch =' + globalState.inBatch, {
        "globalState.inBatch": globalState.inBatch,
        "globalState.isRunningReactions": globalState.isRunningReactions
      })
    ...
    function runReactionsHelper() {
      logger('runReactionsHelper--衍生对象调度',{
        'globalState.pendingReactions':  globalState.pendingReactions
      } )
    ...
    function reaction(expression, effect, opts) {
      logger('reaction--对象创建', {
        expression, effect, opts
      })
      if (opts === void 0) {
        opts = EMPTY_OBJECT;
      }
    ...
    function extendObservable(target, properties, annotations, options) {
      ...
      try {
        ...
        getPlainObjectKeys(descs).forEach(function (key) {
          makeProperty(adm, target, key, descs[key], !annotations ? true : key in annotations ? annotations[key] : true, true, !!(options !== null && options !== void 0 && options.autoBind));
        });
      } finally {
        endBatch();
      }
    ...
    var objectProxyTraps = {
      ...
      get: function get(target, name) {
        ...
        var adm = getAdm(target);
         logger('objectProxyTraps.get---'+ adm.name_)
    ...
    autoBind) {
      logger('【makeProperty遍历属性】' + key+ ' 类型: '+ annotation.annotationType_, {
        key,
        adm, owner, key, descriptor, annotation, forceCopy,
      // extend observable will copy even unannotated properties
      autoBind
      })
        ...
        case OBSERVABLE_STRUCT:
          {
            ...
            if ( !("value" in descriptor)) die("Cannot decorate '" + key.toString() + "': observable cannot be used on setter / getter properties.");
    ...
    function makeObservable(target, annotations, options) {
      logger('makeObservable', {
        start: '这是入口',
        target, annotations, options
      })
      var autoBind = !!(options !== null && options !== void 0 && options.autoBind);
    ...
    function extractAnnotationsFromProto(proto, collector, options) {
      Object.entries(getOwnPropertyDescriptors(proto)).forEach(function (_ref2) {
        ...
        if (prop.get) {
          ...
          collector[key] = isGenerator(prop.value) ? flow : options !== null && options !== void 0 && options.autoBind ? autoAction.bound : autoAction;
        }
      });
    }
    ...
    var ObservableObjectAdministration = /*#__PURE__*/function () {
      ...
      function ObservableObjectAdministration(target_, values_, name_, defaultEnhancer_) {
         logger('ObservableObjectAdministration--adm对象创建', {
          target_,
          values_,
           name_,
            defaultEnhancer_
      ...
      _proto.read_ = function read_(key) {
        ...
        const value = this.values_.get(key).get();
        // logger('读取属性'+ key, value)
        return value
      };
      _proto.write_ = function write_(key, newValue) {
        logger('adm.write_--设置属性'+ key, newValue)
      ...
      _proto.has_ = function has_(key) {
        ...
        if (entry) return entry.get();else {
          ...
          logger('ObservableObjectAdministration.has')
      ...
      _proto.addObservableProp_ = function addObservableProp_(propName, newValue, enhancer) {
        logger('addObservableProp_添加OV属性--'+ propName, {
          propName, newValue, enhancer
        })
        if (enhancer === void 0) {
        ...
        var observable = new ObservableValue(newValue, enhancer, this.name_ + "." + stringifyKey(propName), false, this.name_);
        ...
        logger('defineProperty--重置属性' + propName, {
          target,
          propName,
          config:  generateObservablePropConfig(propName)
        })
    ...
    function asObservableObject(target, name, defaultEnhancer) {
      logger('asObservableObject', {
        target, name, defaultEnhancer
      })
      if (name === void 0) {
        name = "";
      }
