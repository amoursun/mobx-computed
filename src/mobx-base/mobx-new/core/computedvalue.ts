...
export class ComputedValue<T> implements IObservable, IComputedValue<T>, IDerivation {
    ...
    constructor(options: IComputedValueOptions<T>) {
        ...
        this.name_ =
            options.name || (window.__DEV__ ? 'ComputedValue@' + getNextId() : 'ComputedValue');
        if (options.set) {
            this.setter_ = createAction(
                window.__DEV__ ? this.name_ + '-setter' : 'ComputedValue-setter',
                options.set
            ) as any;
        }
        this.equals_ =
    ...
    suspend_() {
        if (!this.keepAlive_) {
            ...
            if (window.__DEV__ && this.isTracing_ !== TraceMode.NONE) {
                console.log(
