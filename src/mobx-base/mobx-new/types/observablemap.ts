    ...
    implements Map<K, V>, IInterceptable<IMapWillChange<K, V>>, IListenable
{
    ...
    constructor(
        ...
        public name_ = window.__DEV__ ? 'ObservableMap@' + getNextId() : 'ObservableMap'
    ) {
        if (!isFunction(Map)) {
            die(18);
        }
        this.keysAtom_ = createAtom(
            window.__DEV__ ? `${this.name_}.keys()` : 'ObservableMap.keys()'
        );
        this.data_ = new Map();
    ...
    has(key: K): boolean {
        ...
        if (!entry) {
            const newEntry = (entry = new ObservableValue(
                this.has_(key),
                referenceEnhancer,
                window.__DEV__ ? `${this.name_}.${stringifyKey(key)}?` : 'ObservableMap.key?',
                false
            ));
