    ...
    implements IInterceptable<IObjectWillChange>, IListenable
{
    ...
    write_(key: PropertyKey, newValue) {
        ...
        console.log('write_', key, newValue, hasInterceptors(this), hasListeners(this));
        // intercept
        ...
        if (newValue !== globalState.UNCHANGED) {
            ...
            if (window.__DEV__ && notifySpy) spyReportStart(change!);
    ...
    addObservableProp_(
        ...
        if (window.__DEV__) assertPropertyConfigurable(target, propName);
        ...
        console.log('addObservableProp_', newValue, `${this.name_}.${stringifyKey(propName)}`);
    ...
    addComputedProp_(
        ...
        options.context = this.proxy_ || target; // 目标实例对象
        // new ComputedValue(options) 实例set到adm.values_上
        ...
        /**
         * 添加属性监听
         * generateComputedPropConfig(propName): 不可枚举的  (遍历工具访问不到)
         * {
                configurable: true,
                enumerable: false,
                get: fn
    ...
    getKeys_(): PropertyKey[] {
        ...
        if (window.__DEV__) Object.freeze(this.keysValue_);
...
export function generateComputedPropConfig(propName) {
    // Object.create(null) => computedPropertyConfigs
    // 开始computedPropertyConfigs 无任何属性和原型的对象
    return (
